...
def correct_humidity(humidity, temperature, corr_temperature):
    global humidity_vals, num_vals
    dewpoint = temperature - ((100 - humidity) / 5)
    corr_humidity = 100 - (5 * (corr_temperature - dewpoint))
    if len(humidity_vals) > num_vals:
        humidity_vals = humidity_vals[1:] + [corr_humidity]
    else:
        humidity_vals.append(corr_humidity)
        
    mean_humidity = numpy.mean(humidity_vals)
    return min(100, mean_humidity)


def analyse_pressure(pressure, t):
    global time_vals, pressure_vals, trend, num_vals
...
with open("/etc/enviro/config.yaml") as f:
...
cpu_temps = [get_cpu_temperature()] * 10
...
while True:
    ...
    mean_pressure, _, trend = analyse_pressure(pressure, now)
